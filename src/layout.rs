...
impl<'a> LayoutBox<'a> {
    ...
    fn calculate_inline_width(&mut self, b_box: Dimensions) {
        ...
        dimensions.content.width = get_absolute_num(style, b_box, "width").unwrap_or(0.0);
        dimensions.margin.left = style.num_or("margin-left", 0.0);
        dimensions.margin.right = style.num_or("margin-right", 0.0);
        dimensions.padding.left = style.num_or("padding-left", 0.0);
        dimensions.padding.right = style.num_or("padding-right", 0.0);
        dimensions.border.left = style.num_or("border-left-width", 0.0);
        dimensions.border.right = style.num_or("border-right-width", 0.0);
    }
    ...
    fn calculate_inline_position(&mut self, b_box: Dimensions) {
        ...
        dimensions.margin.top = style.num_or("margin-top", 0.0);
        dimensions.margin.bottom = style.num_or("margin-bottom", 0.0);
        dimensions.border.top = style.num_or("border-top-width", 0.0);
        dimensions.border.bottom = style.num_or("border-bottom-width", 0.0);
        dimensions.padding.top = style.num_or("padding-top", 0.0);
        dimensions.padding.bottom = style.num_or("padding-bottom", 0.0);

        dimensions.content.x = b_box.content.x
            + b_box.current.x
            + dimensions.margin.left
            + dimensions.border.left
            + dimensions.padding.left;
        dimensions.content.y = b_box.content.y
            + b_box.content.height
        ...
        match (width, margin_l, margin_r) {
            (0.0, _, _) => {
                if underflow >= 0.0 {
                    ...
                    dimensions.margin.right = margin_r_num + underflow;
                }
                dimensions.margin.left = margin_l_num;
            }
            ...
            (w, None, None) if w != 0.0 => {
                dimensions.content.width = w;
                dimensions.margin.left = underflow / 2.0;
                dimensions.margin.right = underflow / 2.0;
            }
            (_, _, _) => {
                dimensions.content.width = width;
                dimensions.margin.left = margin_l_num;
    ...
    fn calculate_position(&mut self, b_box: Dimensions) {
        ...
        dimensions.content.x = b_box.content.x
            ...
            + dimensions.border.left
            + dimensions.padding.left;
        dimensions.content.y = b_box.content.height
            + b_box.content.y
            + dimensions.margin.top
            + dimensions.border.top
            + dimensions.padding.top;
